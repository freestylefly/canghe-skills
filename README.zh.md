...
苍何分享的 Claude Code 技能集，提升日常工作效率。
...
# 发布带图片
/canghe-post-to-x "看看这个" --image photo.png
...
```bash
# 添加到 .canghe-skills/.env（项目级）或 ~/.canghe-skills/.env（用户级）
...
```bash
# 基础生成（自动检测服务商）
/canghe-image-gen --prompt "一只可爱的猫" --image cat.png

# 指定宽高比
/canghe-image-gen --prompt "风景图" --image landscape.png --ar 16:9

# 高质量（2k 分辨率）
/canghe-image-gen --prompt "横幅图" --image banner.png --quality 2k

# 指定服务商
/canghe-image-gen --prompt "一只猫" --image cat.png --provider openai

# DashScope（阿里通义万相）
/canghe-image-gen --prompt "一只可爱的猫" --image cat.png --provider dashscope

# 带参考图（仅 Google 多模态支持）
/canghe-image-gen --prompt "把它变成蓝色" --image out.png --ref source.png
```
...
```bash
/canghe-danger-gemini-web "你好，Gemini"
/canghe-danger-gemini-web --prompt "解释量子计算"
```
...
提取微信公众号文章（mp.weixin.qq.com）的元数据和内容。支持提取标题、作者、摘要、发布时间以及文章正文。
...
将 Markdown 文件转换为完全适用微信公众号排版的优雅 HTML。支持代码高亮、数学公式、PlantUML、脚注、提示块和信息图表。

```bash
# 基础转换（使用默认主题）
/canghe-markdown-to-html article.md

# 指定主题（default, grace, simple）
/canghe-markdown-to-html article.md --theme grace
```

#### find-skills

一个面向技能生态的“检索导航器”：先把需求翻译为搜索关键词，再通过 `npx skills` 定位候选技能，并给出可直接执行的安装命令。

#### obsidian-skills

围绕 Obsidian 知识管理场景的综合技能包，覆盖笔记写作、结构化视图和 Canvas 组织等常用能力。

#### obsidian-bases

用于构建和维护 Obsidian Bases（`.base`）文件，支持多视图配置、过滤逻辑、公式字段与汇总统计，保证 YAML 结构可用。

#### obsidian-markdown

用于稳定处理 Obsidian Flavored Markdown：包括 wikilink、嵌入、callout、frontmatter 元数据、标签等库内语法细节。

#### remotion-best-practices

面向 Remotion + React 视频工程的规则参考集，涵盖时序控制、组合设计、字幕方案、转场实现、素材处理与渲染质量实践。
...
本仓库中不少技能是在宝玉老师的技能基础上进行二次修改和本地化落地，结合了实际工作流、工具链与提示词风格。源仓库地址：[https://github.com/JimLiu/baoyu-skills](https://github.com/JimLiu/baoyu-skills)。  
...
**加载优先级**（高优先级覆盖低优先级）：
1. 命令行环境变量（如 `OPENAI_API_KEY=xxx /canghe-image-gen ...`）
2. `process.env`（系统环境变量）
3. `<cwd>/.canghe-skills/.env`（项目级）
4. `~/.canghe-skills/.env`（用户级）
...
```bash
mkdir -p .canghe-skills
# 将 .canghe-skills/.env 添加到 .gitignore 避免提交密钥
...
**扩展路径**（按优先级检查）：
1. `.canghe-skills/<skill-name>/EXTEND.md` - 项目级（团队/项目特定设置）
2. `~/.canghe-skills/<skill-name>/EXTEND.md` - 用户级（个人偏好设置）

**示例**：为 `canghe-cover-image` 自定义品牌配色：

```bash
mkdir -p .canghe-skills/canghe-cover-image
```

然后创建 `.canghe-skills/canghe-cover-image/EXTEND.md`：
...
```bash
HTTP_PROXY=http://127.0.0.1:7890 HTTPS_PROXY=http://127.0.0.1:7890 /canghe-danger-gemini-web "你好"
```
